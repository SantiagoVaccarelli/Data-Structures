...
bool imprimir_informe(char* clave,void* dato,void* general,size_t n){
...
bool contar_doctores(char* clave,void* dato,void* extra,size_t extra_2){
...
void general_encolar_paciente(general_t* general,char* nombre,char* especialidad,char* urgencia){
    if(!hash_pertenece(general->especialidades,especialidad)) printf(ENOENT_ESPECIALIDAD,especialidad);
        ...
        int anio = *(int*)anio_;
        if (anio>0 && anio<2021) printf(ENOENT_ANIO,anio_);
        if ( true){
    ...
    if(!abb_pertenece(general->doctores,doctor)) printf(ENOENT_DOCTOR,doctor);
        ...
        if(paciente) {
            ...
            if(abb_obtener(general->atendidos,doctor)==0){
                ...
                cant = (int*)abb_obtener(general->atendidos,doctor);
                *cant+=1;
...
void general_crear_informe(general_t* general,char* inicio,char* fin){
    ...
    abb_in_order(general->doctores,contar_doctores,&cant_doctores,inicio,fin);
    printf(DOCTORES_SISTEMA,cant_doctores);
    abb_in_order(general->doctores,imprimir_informe,(void*)general,inicio,fin);
